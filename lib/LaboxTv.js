...
function log(message,level) {
    if (debugMode === true and level > logLevel) {
...
function startWs(ipAddress, port, callback) {
    try {
        ...
        ws.on('close', function() {
            ...
            log('WS on port ' + port + ' closed, restarting',2);
        ...
        ws.on('error', function(err) {
            ...
            log('WS on port ' + port + ' got an error: "' + err +'", restarting',2);
            startWs(ipAddress, port, callback);
        });
        ws.on('open', function() {
            log('WS opened on port ' + port,2);
            if (port === commandWsPort) {
                labox.emit('open');
                // Get TV status
                getLaboxStatus();
        ...
        ws.on('message', function(data) {
            log('Message received on port ' + port + ': ' + data),5;
        ...
        log('Error while starting WS on port ' + port + ': ' + e.message,1);
...
function getPlayerInfoFromMessage(port, message) {
    try {
            ...
            if (jsonMessage.hasOwnProperty('Action')) {
                ...
                if (jsonMessage.hasOwnProperty('Data')) {
                    ...
                    switch(action) {
                        ...
                        default:
                            log('No treatment for WS command message with action "' + action + '": ' + message,1);
                            break;
                    }
                } else {
                    log('Message from command WS with no data:' + message,1);
                }
            } else {
                log('Message from command WS with no action:' + message,1);
            }
        } else {
            if (jsonMessage.hasOwnProperty('Notification')) {
                ...
                if (jsonMessage.hasOwnProperty('Params')) {
                    ...
                    switch(action) {
                        ...
                        default:
                            log('No treatment for WS notification message with action "' + action + '": ' + message,1);
                            break;
                    }
                } else {
                    if (action === 'StatusUpdate') {
                        ...
                        log('Message from notification WS with no data:' + message,1);
                    }
                }
            }
        }
        ...
        log('Error while getting player info from message: ' + e.message,1);
    }
}
...
function setRefreshInterval(interval) {
    ...
    log('Schedule auto refresh data in ' + Math.ceil(interval / 1000) + ' seconds',3);
}

setInterval(function() {
    if (commandWs && commandWs.readyState === WebSocket.OPEN) {
        log('WS on port ' + commandWsPort + ': sending ping',5);
        commandWs.ping();
    }
    if (notificationWs && notificationWs.readyState === WebSocket.OPEN) {
        log('WS on port ' + notificationWsPort + ': sending ping',5);
        notificationWs.ping();
    }
}, 60000);

module.exports = function(ip, debug, level) {
    ipAddress = ip;
